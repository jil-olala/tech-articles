...
- **来源：** 阿里云开发者
- **日期：** 2026-04-28
- **原文链接：** https://mp.weixin.qq.com/s/cIQYl9Wr1Eov4ma-_bYh-w

---

id="js_content" style="visibility: hidden; opacity: 0; ">
![image](images/agent_arch_practice/img-01.jpg)
阿里妹导读
...
![image](images/agent_arch_practice/img-02.jpg)
...
需要中间推理与灵活判断
放在一张图里看，会更直观：
![image](images/agent_arch_practice/img-03.jpg)
**五种常见控制模式**
...
![image](images/agent_arch_practice/img-04.jpg)
...
![image](images/agent_arch_practice/img-05.jpg)
...
**Harness 的关键结论是什么**
![image](images/agent_arch_practice/img-06.jpg)
...
![image](images/agent_arch_practice/img-07.jpg)
...
`;
async function executeLoadSkill(name: string): Promise {
...
![image](images/agent_arch_practice/img-08.png)
...
对应 API 能做的操作
示例
update_yuque_postget_post + update_content + update_title
返回
与下一步决策直接相关的字段
完整原始数据
错误
...
```
![image](images/agent_arch_practice/img-09.jpg)
...
![image](images/agent_arch_practice/img-10.jpg)
...
![image](images/agent_arch_practice/img-11.jpg)
...
![image](images/agent_arch_practice/img-12.jpg)
...
![image](images/agent_arch_practice/img-15.jpg)
**多 Agent 下幻觉会互相放大**
...
![image](images/agent_arch_practice/img-16.jpg)
**子 Agent 的深度限制和最小提示**
...
**为什么 Agent 评测结构更复杂**
![image](images/agent_arch_practice/img-17.png)
...
![image](images/agent_arch_practice/img-18.png)
...
![image](images/agent_arch_practice/img-19.png)
![image](images/agent_arch_practice/img-20.png)
...
![image](images/agent_arch_practice/img-21.png)
...
![image](images/agent_arch_practice/img-22.jpg)
**在线评测如何做采样**
...
![image](images/agent_arch_practice/img-23.jpg)
```
# Agent 执行时 emit 事件
...
![image](images/agent_arch_practice/img-24.jpg)
层
实现
主要职责
关键设计决策
...
class AgentLoop {
  ...
  async runLoop(messages) {
    for (let i = 0; i dispatch 不做 await，不同 session 的消息可以并发处理，互不阻塞，但同一 session 内的消息必须串行，否则并发写历史和触发 compact 会有竞态，生产环境要对每个 sessionKey 维护一个队列或 mutex。
...
![image](images/agent_arch_practice/img-25.jpg)
...
async function saveProgress(state: TaskState): Promise {
  const path = `.openclaw/tasks/${state.taskId}.json`;
  await fs.writeFile(path, JSON.stringify(state, null, 2));
}
async function resumeTask(taskId: string): Promise {
  try {
...
async function handleMessage(msg: InboundMessage): Promise {
...
async function executeShell(args: string[], cwd?: string): Promise {
...
```
async function auditedShell(args: string[], userId: string): Promise {
  // 执行前记录：时间、用户、命令
  await fs.appendFile(
