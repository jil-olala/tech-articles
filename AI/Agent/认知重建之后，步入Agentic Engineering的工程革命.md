...
author: ""
date: 2026-03-30
original_link: https://mp.weixin.qq.com/s/ri_lxDGayM-e5A0oAW59Fw
category: AI/Agent
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_gif/j3gficicyOvasVeMDmWoZ2zyN8iaSc6XWYj79H3xfgvsqK9TDxOBlcUa6W0EE5KBdxacd2Ql6QBmuhBJKIUS4PSZQ/640?wx_fmt=gif&from=appmsg)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz907rZWbBGwxlC1y33Xm3JQMQqiasxZclacuvmGGyJUEh6ogD57NdJiaSb1TnibjfTI2kZLOl83VGldqSibJOvRaKd950HwQyVuI0xr4/640?wx_fmt=png&from=appmsg)
...
![image](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz907evibw6WfZCzHWgKiawrsVVqwhgfiarmCG8TjlK8qAOBO58HWZf84nvoSyRqt08AURMAx27k3geUJgeDeH8RbhfPnVZaL4mluJuQ/640?wx_fmt=png&from=appmsg)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz906Vficicrm1g2ANMW2KPukY495v4hl3vRZSVcORVUWwSpOE29GMf8rzGSKatcCe25KiagCdktTBj7hHBSQ7DHPRiaT6NVuFibIIWFtM/640?wx_fmt=png&from=appmsg)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz907VNdTHraRBjYjXnuBX01d3w4G3m9WWBQibe9F4bVXia6cpib3XmqTp5dD1EuD2TknTaNLTQviaZBh4ibQo864BYFWibGibItDVKx8xyE/640?wx_fmt=png&from=appmsg)
...
![image](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz9056WYLicbXQFU0prQibicbGx7ruSaiacBicDQT7pky4t1PPymoSBjZ9VqDAJDunibeaiaFnVYPI1HkOsMc2HRePmfb05APB87oDmmEC1I/640?wx_fmt=png&from=appmsg)
...
![image](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz905CJ3R7sicdR0ubYibsg1O8TztqlJ8cqMXdibbEfmvdvl94MFyvPSenibGIefEico6AVm0GQULyNicWaVmQibycRdvzORDuiaHchMbHKPw/640?wx_fmt=png&from=appmsg)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz907V4DQU1eZnXG5AxvNINWv6UxcrMdC9NpmyibBhf7JIQNU92qF8JRdHqzhBichRB82lCVic612kOZgGuMgYRLBJtMNP75akFOvCcA/640?wx_fmt=png&from=appmsg)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz906VM2OTPh9Piao4VBzEJ1IWLicrQmN8MyqfMicJyuibTNRjEAeBzUTD6I37jMa8LvEvzt5O44rbLhjTOy0Chm5DnqsJhSv2Golen6M/640?wx_fmt=png&from=appmsg)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz905g5ZqynygmexViaG1Vm3TVqgHFCdorHTGueMLJXHhEgkkaHH40NyYjVYpr4YSR9u9Po6JqNLzeiaYbGfAGICLquksT6BcIqchHU/640?wx_fmt=png&from=appmsg)
...
-
**预检**（code-review-prepare Skill）：自动识别当前需求、发现待审查的服务仓库、获取增量变更、确认审查范围

-
**并行审查**：最多 7 个专项 checker 同时工作，各管一个维度

-
**汇总报告**（code-review-report Skill）：合并所有结果，生成统一的审查报告

**7 个专项 checker 各司其职**：

![image](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz9040ibumXUdDeg0zib7kMzHm58tfH07RguMB7diarxhpV4tN7egiaCuusib9WEc23UnS24lzLh9AuJiaMnbqaxbF0hNIKh8AExCAichhaQ/640?wx_fmt=png&from=appmsg)
...
-
**商品配置**（雅典娜平台）：定义奖品是什么——现金券、积分、优惠券、玩法道具

-
**活动配置**（雅典娜平台）：定义活动本身——名称、时间、投放渠道、关联商品、库存锁定

-
**玩法规则配置**（Apollo 控制台）：定义用户如何参与——When 条件触发 → Then 动作执行（发奖、创建任务、抽奖）
...
```
用户下达任务 Prompt     │     ▼┌─────────────────────────────────────┐│  Agent 执行任务（编码/配置/测试）      │◀──────┐│  完成后准备退出会话                    │       │└─────────────┬───────────────────────┘       │              │ 触发 Stop Hook                 │              ▼                                │┌─────────────────────────────────────┐       ││  检查完成条件：                        │       ││  · 输出了 完成标记？│       ││  · 达到最大迭代次数？                  │       │└─────┬──────────────────┬────────────┘       │      │ 未完成            │ 已完成              │      ▼                  ▼                     │  阻止退出           允许退出                    │  把原始 Prompt                                 │  重新注入为                                    │  新的用户消息 ─────────────────────────────────┘
```
...
-
**完成条件由 Agent 自主判断。** 当 Agent 确认任务真正完成时，输出一个特定标记（...），Stop Hook 检测到后允许退出。如果 Agent 没有完成，循环继续
...
![image](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz906EblDnvwlR0WfZjtSSwDJ96Yrz5bpSOv54tEic5quKVrDdaibXLprD1mxRLglnZJWlibA6jvQS57JktIDazAl8VPo4zCGQhKDsC4/640?wx_fmt=png&from=appmsg)
...
![image](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz905mIQ1X7NAn3ro0f0wdmibCsxcdeMePQg5TQhciaqpFOHAWuQQ91Yo37icKRblicIfhicfpr5558LpJt2qjYy0ibnS6RenKvg3Kq7B9o/640?wx_fmt=png&from=appmsg)
...
-
**打开浏览器并登录**：playwright-cli open <雅典娜测试环境URL> --headed，通过 snapshot 定位"快速登录"按钮并点击，完成 OA 单点登录。

-
**商品页验证**：导航到"商品配置"菜单，以 goods_id 搜索，断言商品名称、类型（type_property/玩法道具）、状态（released/已发布）与预期一致；打开详情弹窗，断言 sub_type、count_in_stock、value 等字段值正确。

-
**活动页验证**：进入"运营配置 → 运营活动"，以 act_id 搜索，断言活动名称、状态、时间范围；在活动详情页 reload 后监听 Network 请求，确认 act_info、select_info、goods_list 三个接口均返回 200，且响应中 game_info.game_detail 非空、awards[].package[].package_mall[].goods_id 与之前创建的商品 ID 匹配。

-
**截图留证**：对商品列表、活动列表、活动详情页分别截图存档。
...
-
goods_addgoods_modifygoods_query：商品的创建、修改、查询

-
stock_lock / stock_unlock：库存锁定和释放

-
act_createact_updateact_query：活动的创建、更新、查询
...
![image](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz9049vg7VfQoNXd14tnUico0ia6nMGibqcQW8lPRlj4CBbL2CVNAgAicO6OM55VYS29FZStCiamA6cICYWohciaWScoXzUlQJKsTFyz9ys/640?wx_fmt=png&from=appmsg)
...
-
提供业务上下文——Tapd 链接、业务规则、关注的代码位置

-
让 AI 按规范推进——它会自动执行阶段检查、更新状态

-
验证与纠偏——检查输出，发现问题就反馈

-
确认沉淀——notes.md 和 context/ 是否更新了
...
-
**选一个具体需求**，和项目负责人一起用 Agentic 流程推进——不是演示，是真做

-
**在做的过程中积累项目上下文**——该项目的服务架构、业务规则、技术约束，沉淀到 context/project/ 下

-
**跑通一个完整闭环后撤出**——项目团队已经有了第一份业务知识库，后续可以自己迭代
...
-
读《Agentic 工程师入门指南》（30 分钟）

-
在一位老成员的陪伴下跑通第一个 /requirement:new（30 分钟）

-
独立完成一个小需求（1-2 天）

-
完成后复盘：哪里卡住了？哪里 AI 犯错了？记录到 notes.md
...
复杂需求也能高效完成，Project 级上下文开始丰富
**精通**
3+ 月

能优化团队规范、设计可复用的 Skill、帮助团队成员上手
...
![image](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz904QIIb1vUs2KNziboL4ibaP4MBSfF8Pmx0kAzNLH72oIWZwOHuzOguALaibhMgia7oGmiagsEes5uCgNSST4ibstVmC2pxAbNuzPMiar8/640?wx_fmt=png&from=appmsg)
...
-
**大多数项目，单 Agent + 结构化上下文就够了**——个人开发者和小团队不需要模拟完整团队，需要的是让 Agent 在每个环节都拿到精准的上下文，一次做对。

-
**关键环节按需引入角色化**——需求分析、代码审查这些确实需要不同视角的环节，再引入专门的 Agent。不是"先配齐团队再开工"，而是"需要时再拉人"。

-
**并行用在执行层，不是决策层**——代码审查 6 个 Checker 并行、批量测试并行，这些执行性工作的并行收益最直接。但方案设计、架构决策这些需要深度思考的环节，并行反而带来冲突。
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_gif/j3gficicyOvasVeMDmWoZ2zyN8iaSc6XWYjZ7Hx6Udjjk2BGLzC9ahJq7ibxDd1RGA0c9NYZc1husEsvb3tY4FcWPQ/640?wx_fmt=gif&from=appmsg)

![image](https://mmbiz.qpic.cn/sz_mmbiz_png/j3gficicyOvasVeMDmWoZ2zyN8iaSc6XWYj5q5PQEOc5ibURPb03vnRibrxC3UR8xzdyATfiawTYRV2vJvBnAIcE1FeQ/640?wx_fmt=png&from=appmsg)
